...
# FIX: import real retrieval metrics module
from pipeline.retrieval_metrics import compute_retrieval_metrics, compute_ss2fd, compute_qcs
...
def _parse_file(filename: str, content: bytes) -> str:
    ...
    if ext == "pdf":
        try:
            ...
            return "\n\n".join(_clean_pdf_page(p) for p in pages if p.strip())
        except Exception:
            pass
        try:
            import pdfplumber
...
def _validate_user_config(user_config: Dict[str, Any]) -> Dict[str, Any]:
    from pipeline.s2_chunkers import VALID_STRATEGIES
    cfg = dict(user_config or {})
    numeric_ranges = {
        "n_min": (20, 400), "n_max": (80, 1200),
        "tau_jsd_low": (0.01, 0.6), "tau_jsd_high": (0.05, 0.95),
        "tau_sem": (0.2, 0.99), "max_iterations": (1, 30),
        "alpha": (0.0, 1.0), "beta": (0.0, 1.0),
        "lambda": (0.0, 1.0), "hybrid_lambda": (0.0, 1.0),
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# FIX 3: Strategy deduplication helper
# ─────────────────────────────────────────────────────────────────────────────

def _boundary_hash(chunks: List[Dict]) -> str:
    """Hash a chunk set by its sorted boundary positions for deduplication."""
    key = "|".join(f"{c.get('start', 0)},{c.get('end', 0)}" for c in chunks)
...
def _span_coverage(chunks: List[Dict], doc_len: int) -> float:
    if not chunks or doc_len <= 0:
        return 0.0
    covered = sum(max(0, int(c.get("end", 0)) - int(c.get("start", 0))) for c in chunks)
    return min(1.0, covered / doc_len)


def _span_non_redundancy(chunks: List[Dict], doc_len: int) -> float:
    if not chunks or doc_len <= 0:
        return 1.0
    intervals = sorted((int(c.get("start", 0)), int(c.get("end", 0))) for c in chunks)
    overlap = 0
    prev_end = 0
    ...
    total = sum(max(0, e - s) for s, e in intervals)
    return 1.0 - min(1.0, overlap / max(total, 1))
...
def _average_chunk_separation(chunks: List[Dict]) -> float:
    ...
    for i in range(len(chunks) - 1):
        v1 = _embed(chunks[i].get("text", ""))
        v2 = _embed(chunks[i + 1].get("text", ""))
        n1, n2 = np.linalg.norm(v1), np.linalg.norm(v2)
        if n1 > 0 and n2 > 0:
            seps.append(float(np.clip(1.0 - np.dot(v1, v2) / (n1 * n2), 0.0, 1.0)))
    return float(np.mean(seps)) if seps else 0.5
...
def _stage2_boundary_quality(chunks: List[Dict]) -> float:
    ...
    for chunk in chunks:
        ...
        if not words:
            good += 0.5
            continue
        start_ok = bool(words[0]) and (words[0][0].isupper() or bool(re.match(r"^\d", words[0])))
        end_ok   = bool(text) and (
            text[-1] in {".", "!", "?", ":", ";", "}", "]", "`"}
            or len(words) < 30
        )
...
def _stage2_token_distinctness(chunks: List[Dict]) -> float:
    ...
    def _content_set(text: str) -> set:
        return {
            tok for tok in re.findall(r"\b[\wÀ-ÿ]{3,}\b", text.lower())
            if tok not in stop and not tok.isdigit()
        }
    vals = []
    for idx in range(len(chunks) - 1):
        a = _content_set(chunks[idx].get("text", ""))
        b = _content_set(chunks[idx + 1].get("text", ""))
        if not a or not b:
            vals.append(0.7)
            continue
...
def _score_stage2_strategy(
    ...
    empty = {
        "strategy": strategy_name, "rank": 0, "winner": False, "score": 0.0,
        "chunk_count": 0, "mean_tokens": 0, "mean_icc": 0.0, "inter_separation": 0.0,
        "entropy_strength": 0.0, "boundary_distinctiveness": 0.0, "size_fit": 0.0,
        "coverage": 0.0, "count_fit": 0.0, "boundary_quality": 0.0,
        "structure_integrity": 0.0, "cohesion": 0.0, "non_redundancy": 0.0,
        "distinctness": 0.0, "chunking_time": 0.0, "time_efficiency": 0.0,
    }
    if not chunks:
        return empty
    ...
    score = (
        0.23 * size_fit + 0.18 * count_fit + 0.17 * boundary_quality
        + 0.14 * structure_integrity + 0.12 * cohesion
        + 0.09 * distinctness + 0.04 * coverage + 0.03 * time_efficiency
    )
    return {
        "strategy": strategy_name, "rank": 0, "winner": False,
        "score": round(float(max(0.0, min(1.0, score))), 4),
        "chunk_count": len(chunks), "mean_tokens": round(mean_tokens, 1),
...
def _evaluate_stage2_outputs(
    ...
    for name, chunks in outputs.items():
        ...
        rows.append(row)
        scores[name] = {
            "score": row["score"], "chunk_count": row["chunk_count"],
            "mean_tokens": row["mean_tokens"], "size_fit": row["size_fit"],
            "coverage": row["coverage"], "count_fit": row["count_fit"],
            "boundary_quality": row["boundary_quality"],
            "structure_integrity": row["structure_integrity"],
            "cohesion": row["cohesion"], "distinctness": row["distinctness"],
...
# ─────────────────────────────────────────────────────────────────────────────
# FIX 4+5+6: _score_strategy with end-chunk exclusion + real retrieval metrics
# ─────────────────────────────────────────────────────────────────────────────
...
def _score_strategy(
    ...
    if not chunks:
        return {
            "strategy": strategy_name, "rank": 0, "winner": False, "score": 0.0,
            "chunk_count": 0, "mean_tokens": 0, "mean_icc": 0.0,
            "inter_separation": 0.0, "entropy_strength": 0.0,
            "boundary_distinctiveness": 0.0, "size_fit": 0.0,
        }

    # FIX 4: exclude end-boundary chunk from all aggregate computations.
    # C_end has zeroed signal fields by design and deflates every metric.
    real_chunks = [c for c in chunks if c.get("boundary_type") != "end"]
    if not real_chunks:
        real_chunks = chunks

    sizes = [max(1, len(c.get("text", "").split())) for c in real_chunks]
    mean_tokens = sum(sizes) / len(sizes)
    mean_icc = sum(float(c.get("icc", 0.5)) for c in real_chunks) / len(real_chunks)
    entropy_strength = sum(
        min(float(c.get("jsd_score", c.get("metric_score", 0.0))) * 2.0, 1.0)
        for c in real_chunks
    ) / len(real_chunks)
    boundary_distinctiveness = sum(
        1.0 - float(c.get("boundary_score", 0.5)) for c in real_chunks
    ) / len(real_chunks)
    inter_separation = _average_chunk_separation(real_chunks)
    ...
    # FIX 5+6: real retrieval metrics (BM25 leave-one-out, zero LLM)
    embeddings      = output.get("embeddings") if isinstance(output, dict) else None
    ret             = compute_retrieval_metrics(real_chunks, k=5, embeddings=embeddings)
    ret_cost        = mean_tokens * len(real_chunks)
    ss2fd_val       = compute_ss2fd(real_chunks, embeddings) if embeddings else 0.0
    qcs_val         = compute_qcs(real_chunks, ss2fd_val, ret_cost)
    ...
    row: Dict[str, Any] = {
        ...
        "winner": False,
        "score": round(float(max(0.0, min(1.0, score))), 4),
        "chunk_count": len(chunks),   # full count including end-chunk
        ...
        "size_fit": round(float(max(0.0, min(1.0, size_fit))), 4),
        "ss2fd": round(ss2fd_val, 4),
        "qcs": round(qcs_val, 4),
        "retrieval_token_cost": round(ret_cost, 1),
    }

    # Only include MRR/NDCG/Precision/Recall when actually computed
    if ret.get("computed"):
        row.update({
            "mrr":               ret["mrr"],
            "ndcg":              ret["ndcg"],
            ...
            "retrieval_k":       ret["k"],
            "retrieval_n_queries": ret["n_queries"],
        })
...
def _evaluate_strategy_outputs(
    ...
    for name, output in outputs.items():
        ...
        row = _score_strategy(name, chunks, config, output=output)  # FIX: pass output
        rows.append(row)
        scores[name] = {
            "score": row["score"],
...
def _evaluate_s7_outputs(
    ...
    for name, output in outputs.items():
        ...
        # FIX: add ss2fd + QCS to S7 rows (post-RL retrieval quality signals)
        real_chunks_s7 = [c for c in chunks if c.get("boundary_type") != "end"] or chunks
        s7_embeddings  = output.get("embeddings")
        ret_cost_s7    = row["mean_tokens"] * row["chunk_count"]
        ss2fd_s7 = compute_ss2fd(real_chunks_s7, s7_embeddings) if s7_embeddings else 0.0
        qcs_s7   = compute_qcs(real_chunks_s7, ss2fd_s7, ret_cost_s7)
        row["ss2fd"] = round(ss2fd_s7, 4)
        row["qcs"]   = round(qcs_s7, 4)
        ret_s7 = compute_retrieval_metrics(real_chunks_s7, k=5, embeddings=s7_embeddings)
        if ret_s7.get("computed"):
            row.update({
                "mrr":              ret_s7["mrr"],
                "ndcg":             ret_s7["ndcg"],
                "precision":        ret_s7["precision"],
                "recall":           ret_s7["recall"],
                "retrieval_method": ret_s7["method"],
            })
...
def _run_s3_s6_for_strategy(
    ...
    # FIX 1: read stage-level metadata from _stage_meta key set by s3_entropy.py
    # This prevents s3_stats/thresholds being null on 18/19 chunks
    _stage_meta = (refined[-1].pop("_stage_meta", {}) or {}) if refined else {}
    s3_stats   = _stage_meta.get("s3_stats",   refined[-1].get("s3_stats",   {}) if refined else {})
    thresholds = _stage_meta.get("thresholds", refined[-1].get("thresholds", {}) if refined else {})
    ...
    embedded, embeddings = embed_chunks(
        enriched, text, doc_profile,
        work_config["embedding_model"], work_config,
    )
    ...
    details = {
        ...
        "thresholds":          thresholds,   # FIX 1: from _stage_meta
...
# ─────────────────────────────────────────────────────────────────────────────
# Main pipeline runner (background thread)
# ─────────────────────────────────────────────────────────────────────────────
...
def _run_pipeline(job_id: str, doc_id: str, user_config: Dict[str, Any]) -> None:
    try:
        ...
        if not text.strip() and doc.get("raw_content") is not None:
            _update_job(job_id, stage="S1", progress=2, message="Extracting text from uploaded file…")
            parse_start = time.perf_counter()
            text = _parse_file(doc.get("filename") or "upload.txt", doc["raw_content"])
            doc["content"] = text
            doc["token_count"] = len(text.split())
            logger.info(json.dumps({
                "job_id": job_id, "event": "document_parsed",
        ...
        # ── S1 ─────────────────────────────────────────────────────────────
        _update_job(job_id, stage="S1", progress=5, message="Profiling document…")
        ...
        # ── S2 ─────────────────────────────────────────────────────────────
        _update_job(job_id, stage="S2", progress=15, message="Running parallel chunkers…")
        try:
        ...
        initial_chunks = (
            evaluating.get(selected_strategy)
            or select_best_strategy(evaluating, doc_profile["type"], config)
            or next(iter(evaluating.values()))
        )
        ...
        # ── S3-S6 ──────────────────────────────────────────────────────────
        _update_job(job_id, stage="S3-S6", progress=28, message="Running S3-S6 for every chunking method…")

        strategy_outputs: Dict[str, Dict[str, Any]] = {}
        total_methods = max(len(evaluating), 1)

        # FIX 3: deduplicate strategies with identical boundary sets
        ...
        for idx, (name, chunks) in enumerate(evaluating.items(), start=1):
            ...
            bh = _boundary_hash(chunks)
            if bh in seen_hashes:
                original = seen_hashes[bh]
                logger.info(
                    "Strategy '%s' = identical boundaries to '%s' — reusing S3-S6 output",
                    name, original,
                )
                strategy_outputs[name] = {
                    "chunks":     [{**c, "method": name} for c in strategy_outputs[original]["chunks"]],
                    "embeddings": strategy_outputs[original]["embeddings"],
                    "details":    {**strategy_outputs[original]["details"], "strategy": name},
                }
            else:
                seen_hashes[bh] = name
                strategy_outputs[name] = _run_s3_s6_for_strategy(name, chunks, text, doc_profile, config)

        # ── S8: Strategy evaluation ─────────────────────────────────────────
        _update_job(job_id, stage="S8", progress=68, message="Evaluating all chunking methods…")
        ...
        s3_s6_details = {name: output["details"] for name, output in strategy_outputs.items()}
        entity_graphs = {name: output["details"].get("entity_graph", {}) for name, output in strategy_outputs.items()}

        stage_details = {
            **job_store[job_id].get("stage_details", {}),
            "s3": {
                "jsd_series":  winning_output["details"]["jsd_series"],
                "chunk_count": winning_output["details"]["s3_chunk_count"],
                "stats":       winning_output["details"]["s3_stats"],
                "thresholds":  winning_output["details"]["thresholds"],
            },
            "s4": {"chunk_count": winning_output["details"]["s4_chunk_count"]},
            "s5": {
                "entity_graph":  winning_output["details"]["entity_graph"],
                "entity_count":  winning_output["details"]["entity_count"],
                "entity_graphs": entity_graphs,
            },
            "s6": {
                "embedding_dim": winning_output["details"]["embedding_dim"],
                "chunk_count":   winning_output["details"]["chunk_count"],
        ...
        _update_job(job_id, stage_details=stage_details)

        # ── S7: RL calibration ─────────────────────────────────────────────
        _update_job(job_id, stage="S7", progress=72, message="Running RL hyperparameter calibration…")

        best_chunks, reward_history, final_config = run_rl_loop(
            text, doc_profile, winning_output.get("chunks", []), config,
        )
        ...
        for name, res in per_strat.items():
            ...
            s7_outputs[name] = {
                ...
                "reward_breakdown": res.get("reward_breakdown", {}),  # FIX: now populated
                "iterations":       res.get("n_evals", 0),
                "final_reward":     res.get("best_score", 0),
                "embeddings":       strategy_outputs.get(name, {}).get("embeddings"),
            }
        ...
        results = {
            ...
            "reward_history": reward_history,
            "reward_histories": {name: out["reward_history"] for name, out in s7_outputs.items()},
            "strategy_evaluation": evaluation_table,
            "stage2_evaluation":   stage2_table,
            "s7_evaluation":       s7_table,
        }

        _update_job(job_id, status="complete", stage="DONE", progress=100,
                    message="Pipeline complete.", results=results)
        logger.info(f'{{"job_id":"{job_id}","event":"pipeline_complete","chunks":{len(final_chunks)}}}')

    except Exception as exc:
        logger.exception("Pipeline failed")
        _update_job(job_id, status="error", message=str(exc), error=traceback.format_exc())
...
async def run_pipeline(
    ...
    job_store[job_id] = {
        ...
        "message": "Job queued…", "doc_id": document_id,
        "results": None, "error": None, "stage_details": {}, "reward_history": [],
    }
    thread = threading.Thread(
        target=_run_pipeline, args=(job_id, document_id, user_config), daemon=True,
    )
    thread.start()
...
async def export_results(job_id: str, fmt: str) -> Response:
    ...
    if fmt == "csv":
        ...
        writer.writerow(["index", "token_count", "chunk_score", "jsd_score",
        ...
        for c in chunks:
            ...
            writer.writerow([c.get("chunk_index", ""), c.get("token_count", ""),
                              c.get("chunk_score", ""), c.get("jsd_score", ""),
                              c.get("boundary_score", ""), c.get("boundary_type", ""),
                              ents, preview])
        return Response(
    ...
    if fmt == "markdown":
        ...
        lines.append("# AutoChunker Results\n")
